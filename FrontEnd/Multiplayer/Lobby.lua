...
--[[ g_ListTypes disabled - server/friends/history filters not commonly used
local g_ListTypes = {};
...
function UpdateRefreshButton()
	...
	--[[ ListTypeLabel removed (list type dropdown disabled)
...
function PopulateAutoRefreshControls()
	...
	Controls.AutoRefreshButtonLabel:SetText( label );

	Controls.AutoRefreshIntervalPulldown:ClearEntries();
	for i, secs in ipairs( g_AutoRefreshIntervalOptions ) do
		local ct = {};
		Controls.AutoRefreshIntervalPulldown:BuildEntry( "InstanceOne", ct );
		ct.Button:SetText( secs .. "s" );
		ct.Button:SetVoid1( i );
	end
	Controls.AutoRefreshIntervalPulldown:CalculateInternals();
...
function OnAutoRefreshIntervalPull( id )
...
--[[ PopulateListTypePulldown / OnListTypePull disabled - list type dropdown removed
function PopulateListTypePulldown() end
...
function OnUpdate( fDTime )
	if not g_ScreenVisible then return; end
	if not g_AutoRefreshEnabled then return; end
	if Matchmaking.IsRefreshingGameList() then
		g_AutoRefreshTimer = 0;
		return;
	end
	g_AutoRefreshTimer = g_AutoRefreshTimer + fDTime;
	if g_AutoRefreshTimer >= g_AutoRefreshSeconds then
		g_AutoRefreshTimer = 0;
		RefreshGameList();
		UpdateRefreshButton();
	end
